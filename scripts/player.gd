...
const AIR_ACCEL: float = 0.04     # 空中加速度（塔科夫风格，越小惯性越大）
...
const PEEK_DURATION: float = 0.75  # 探头动画时长（秒）
...
var _air_speed_lock: float = 0.0  # 起跳时锁定的速度（空中保持不变）
...
var weapon_ctrl: Node = null  # C# WeaponController
...
# ===== 武器控制器桥接 =====
var reserve_ammo: int = 90    # 保留用于 inventory 参照（C# 实际持有）
var is_reloading: bool = false  # 保留用于武器晃动跳过
...
# ===== 武器系统 =====
var weapon: Node3D  # 当前激活的武器模型节点
var weapon_pivot: Node3D  # 武器挂载点
var _current_weapon_index: int = 0  # 0=AKM, 1=M4A1
var _weapon_model_nodes: Array[Node3D] = []  # 所有已加载的武器模型
var _weapon_ammo_data: Dictionary = {}  # index → {ammo, reserve}, 切枪时记忆备弹
# ===== 武器位置参数（AKM 突击步枪尺寸）=====
...
func _ready() -> void:
	...
	print("[Player] interact_raycast=", interact_raycast)
	...
	# 获取 C# 武器控制器并连接信号
	weapon_ctrl = get_node_or_null("WeaponController")
	if weapon_ctrl:
		weapon_ctrl.connect("fire_mode_changed", Callable(self, "_on_fire_mode_changed"))
		weapon_ctrl.connect("reload_state_changed", Callable(self, "_on_reload_state_changed"))
		weapon_ctrl.connect("shot_fired", Callable(self, "_on_shot_fired"))
...
func setup_hud() -> void:
	...
	health_fill = hud.get_node("HealthPanel/HealthFillClip/HealthFill")
...
func update_fire_mode_display() -> void:
	...
	# 更新显示（从 C# WeaponController 读取模式）
	if weapon_ctrl:
		var mode = weapon_ctrl.get_fire_mode()
		if mode == "AUTO":
...
func update_health_display() -> void:
	...
	if health_fill and max_health > 0.0:
		health_fill.scale.x = clampf(health / max_health, 0.0, 1.0)
	if health_label:
...
func setup_weapon() -> void:
	...
	# 加载所有武器模型
	_weapon_model_nodes.clear()
	
	var akm_node = _load_weapon_model("res://models/Guns/GLB/AKM_v1.0.glb", "AKM_Model")
	if akm_node:
		_weapon_model_nodes.append(akm_node)
	else:
		push_error("AKM 模型加载失败")
	
	var m4a1_node = _load_weapon_model("res://models/Guns/GLB/M4A1.glb", "M4A1_Model")
	if m4a1_node:
		_weapon_model_nodes.append(m4a1_node)
	else:
		push_warning("M4A1 模型未找到，跳过")
	
	# 默认激活 AKM（index=0）
	_current_weapon_index = 0
	_activate_weapon(0)

func _load_weapon_model(glb_path: String, node_name: String) -> Node3D:
	# 如果场景中已有该节点，直接使用
	var existing = weapon_pivot.get_node_or_null(node_name) as Node3D
	if existing:
		disable_all_collisions(existing)
	...
	var y_rot = 0.0
	if node_name.contains("M4A1"):
		y_rot = 90.0
	instance.rotation = Vector3(0, deg_to_rad(y_rot), 0)
	weapon_pivot.add_child(instance)
	instance.visible = false
	disable_all_collisions(instance)
	find_muzzle_point(instance)
	return instance

func _activate_weapon(index: int) -> void:
	# 隐藏所有武器
	for i in range(_weapon_model_nodes.size()):
		if _weapon_model_nodes[i]:
			_weapon_model_nodes[i].visible = false
	
	# 显示目标武器
	if index < 0 or index >= _weapon_model_nodes.size():
		return
	if _weapon_model_nodes[index] == null:
		return
	
	var target = _weapon_model_nodes[index]
	target.visible = true
	target.position = HIP_POSITION
	target.rotation = Vector3(0, deg_to_rad(90.0 if index == 1 else 0.0), 0)
	weapon = target
	base_weapon_pos = HIP_POSITION
	base_weapon_rot = Vector3(0, deg_to_rad(90.0 if index == 1 else 0.0), 0)
	
	# 通知 WeaponController 切换武器，带上记忆的备弹
	if weapon_ctrl:
		var data_path = ""
		match index:
			0: data_path = "res://resources/weapons/AKM.tres"
			1: data_path = "res://resources/weapons/M4A1.tres"
		var wd = load(data_path) as WeaponData if data_path else null
		if wd:
			var saved = _weapon_ammo_data.get(index, {})
			weapon_ctrl.switch_to_weapon(wd, target, saved.get("ammo", -1), saved.get("reserve", -1))
...
func switch_weapon(index: int) -> void:
	...
	# 保存当前武器的备弹
	if weapon_ctrl:
		_weapon_ammo_data[_current_weapon_index] = {
			"ammo": weapon_ctrl._ammo,
			"reserve": weapon_ctrl._reserve_ammo
		}
	
	_current_weapon_index = index
	
	# 播放换枪音效
	var switch_sound = load("res://resources/mp3/换枪.mp3")
	if switch_sound:
		var sp = AudioStreamPlayer.new()
		sp.stream = switch_sound
		sp.bus = "Master"
		add_child(sp)
		sp.play()
...
func toggle_ads(ads_on: bool) -> void:
	...
	base_weapon_rot.x = target_rot.x
	
	# 武器动画（仅影响 X 旋转，保留武器自身的 Y 朝向）
	var tween = create_tween().set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
	tween.tween_property(weapon, "position", target_pos, 0.12)
	tween.tween_property(weapon, "rotation:x", target_rot.x, 0.12)
...
func _input(event: InputEvent) -> void:
	...
	# 切换开火模式（转发到 C#）
	if event.is_action_pressed("toggle_fire_mode") and weapon_ctrl:
		weapon_ctrl.toggle_fire_mode()
	...
	if event is InputEventKey and event.pressed and not event.echo:
		match event.keycode:
			49: switch_weapon(0)  # KEY_1 → AKM
			50: switch_weapon(1)  # KEY_2 → M4A1
	...
	# 正常换弹（转发到 C#）
	if event.is_action_pressed("reload") and weapon_ctrl:
		weapon_ctrl.start_reload()
...
func _physics_process(delta: float) -> void:
	...
	elif _air_speed_lock > 0.0:
		_air_speed_lock = 0.0  # 落地重置
	# 检测刚离地（走边缘掉落时锁定速度）
	if is_on_floor():
		pass
	elif _air_speed_lock <= 0.0:
		_air_speed_lock = max(abs(velocity.x), abs(velocity.z), WALK_SPEED)  # 掉落时锁定当前水平速度
	...
	if is_peeking_left or is_peeking_right:
	...
	# 跑步起跳：1.5倍初速，锁定空中速度
	var sprint_jump := Input.is_action_pressed("sprint") and not is_crouching and not is_ads
	if Input.is_action_just_pressed("jump") and is_on_floor() and not is_crouching:
		velocity.y = JUMP_VELOCITY * (1.5 if sprint_jump else 1.0)
		_air_speed_lock = current_speed  # 锁定起跳时的速度

	# 全自动射击（转发到 C# WeaponController，在明确 is_moving 后）
	if weapon_ctrl:
		weapon_ctrl.tick_fire(is_shooting, is_crouching, is_ads, direction.length() > 0)
	...
	if direction:
		if is_on_floor():
			...
			# 空中：移动键仅产生极微弱影响（SPEED=1.0）
			var target_x = direction.x * 1.0
			var target_z = direction.z * 1.0
			velocity.x = lerp(velocity.x, target_x, AIR_ACCEL)
			velocity.z = lerp(velocity.z, target_z, AIR_ACCEL)
	else:
		if is_on_floor():
...
func get_reserve_ammo() -> int:
	if weapon_ctrl:
		return weapon_ctrl.get_reserve_ammo()
	return reserve_ammo

func add_reserve_ammo(amount: int) -> void:
	if weapon_ctrl:
		weapon_ctrl.add_reserve_ammo(amount)

func set_reserve_ammo(amount: int) -> void:
	if weapon_ctrl:
		weapon_ctrl.set_reserve_ammo(amount)
...
func open_inventory() -> void:
	...
	# 显示 Mica 背景
	var mica = hud.get_node_or_null("MicaBackdrop")
	if mica: mica.visible = true
...
func start_crouch() -> void:
	...
	if _crouch_tween and _crouch_tween.is_valid():
		_crouch_tween.kill()
	_crouch_tween = create_tween().set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_SINE)
	_crouch_tween.set_parallel(true)
	if collision_shape:
		_crouch_tween.tween_property(collision_shape, "shape:height", CROUCH_HEIGHT, CROUCH_DURATION)
	if camera_pivot:
		_crouch_tween.tween_property(camera_pivot, "position:y", CROUCH_EYE_HEIGHT, CROUCH_DURATION)
...
func stop_crouch() -> void:
	...
	_crouch_tween.set_parallel(true)
	if collision_shape:
		_crouch_tween.tween_property(collision_shape, "shape:height", STAND_HEIGHT, CROUCH_DURATION)
	if camera_pivot:
		_crouch_tween.tween_property(camera_pivot, "position:y", STAND_EYE_HEIGHT, CROUCH_DURATION)
...
func start_peek_left() -> void:
	...
	_peek_tween_to(PEEK_TILT_ANGLE, -PEEK_SHIFT_DISTANCE)
...
func _peek_tween_to(target_tilt: float, target_shift: float) -> void:
	if _peek_tween and _peek_tween.is_valid():
		_peek_tween.kill()
	_peek_tween = create_tween().set_ease(Tween.EASE_IN_OUT).set_trans(Tween.TRANS_SINE)
	_peek_tween.set_parallel(true)
	_peek_tween.tween_property(self, "rotation:z", target_tilt, PEEK_DURATION)
	if collision_shape:
		_peek_tween.tween_property(collision_shape, "position:x", target_shift, PEEK_DURATION)
...
func open_search_panel(ammo_box: Node3D) -> void:
	...
	# 显示 Mica 背景
	var smica = hud.get_node_or_null("SearchMicaBackdrop")
	if smica: smica.visible = true
...
func create_ammo_slot(has_item: bool, ammo_box: Node3D, slot_index: int = 0) -> Control:
	...
	var style = StyleBoxFlat.new()
	style.bg_color = Color(0.157, 0.176, 0.255, 0.55)
	style.set_border_width_all(1)
	style.border_color = Color(0.471, 0.706, 1.0, 0.10)
	...
	if has_item:
		...
		name_label.add_theme_color_override("font_color", Color(0.910, 0.929, 0.961))
		...
		count_label.add_theme_color_override("font_color", Color(0.992, 0.737, 0.251))  # 金色
		...
		empty_hint.add_theme_color_override("font_color", Color(0.4, 0.4, 0.5, 0.5))
...
func create_search_slot(slot_data: Dictionary, slot_index: int = 0) -> Control:
	...
	style.bg_color = Color(0.157, 0.176, 0.255, 0.55)
	style.set_border_width_all(1)
	var quality = slot_data.get("quality", "")
	if quality != "":
		style.border_color = _inventory_quality_color(quality)
	else:
		style.border_color = Color(0.471, 0.706, 1.0, 0.10)
	style.set_corner_radius_all(7)
	panel.add_theme_stylebox_override("panel", style)
	...
	if slot_data.get("name", "") != "":
		...
		if count > 1 or slot_data.get("is_ammo", false):
			...
			count_label.add_theme_color_override("font_color", Color(0.992, 0.737, 0.251))
			vbox.add_child(count_label)
...
func _inventory_quality_color(q: String) -> Color:
	match q:
		"green":  return Color(0.212, 0.769, 0.239)
		"blue":   return Color(0.255, 0.553, 0.953)
		"purple": return Color(0.624, 0.392, 0.902)
		_:        return Color(0.376, 0.804, 1.0)
...
func _show_extraction_ui(seconds_remaining: int) -> void:
	...
	# 更新倒计时显示
	extraction_ui_label = hud.get_node_or_null("ExtractionUI/VBox/CountdownLabel")
	extraction_ui_progress = hud.get_node_or_null("ExtractionUI/VBox/CountdownProgress")
...
func _create_extraction_ui() -> Control:
	...
	# 半透明深色背景（亚克力风格）
	var style = StyleBoxFlat.new()
	style.bg_color = Color(0.078, 0.098, 0.157, 0.85)
	style.set_border_width_all(2)
	style.border_color = Color(0.910, 0.067, 0.137, 0.8)
	...
	progress_style.bg_color = Color(0.078, 0.098, 0.157, 0.55)
	progress_style.set_border_width_all(1)
	progress_style.border_color = Color(0.910, 0.067, 0.137, 0.5)
	progress_style.set_corner_radius_all(6)
	progress.add_theme_stylebox_override("background", progress_style)
	
	var fill_style = StyleBoxFlat.new()
	fill_style.bg_color = Color(0.212, 0.769, 0.239, 1.0)
...
func _on_reload_state_changed(reloading: bool) -> void:
	is_reloading = reloading
	if reload_hint:
		reload_hint.visible = reloading
	...
	if weapon:
		...
		if reloading:
			...
			tween.tween_property(weapon, "rotation:x", 0.0, 0.2)

func _on_shot_fired(headshot_mult: float) -> void:
	print("[C#] 爆头！倍率 x", headshot_mult)
	_show_hit_feedback(headshot_mult)
